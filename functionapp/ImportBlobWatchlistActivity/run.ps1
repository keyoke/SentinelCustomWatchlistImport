...
$WatchlistName = [System.IO.Path]::GetFileNameWithoutExtension($blob.Name) -replace 'watchlist_','' -replace '[^a-zA-Z]', ''
...
if($status.Status -eq 'Success')
{
    ...
    Write-Host "Moved file '$($blob.name)' from '$($env:APPSETTING_WATCHLIST_STORAGE_INCOMING_CONTAINER_NAME )' to '$($env:APPSETTING_WATCHLIST_STORAGE_COMPLETED_CONTAINER_NAME)'."
}

Write-Host "Completed Processing file '$($blob.name)'"
